    ...
    <div className="text-foreground flex w-full flex-col gap-8">
      <div className="px-2 py-8">
        <FullBleedLines className="flex w-full justify-between">
          <p className="font-heading w-full max-w-5xl text-2xl leading-none font-bold tracking-tight sm:text-4xl md:text-5xl lg:text-[64px]">
      ...
      <FullBleedLines>
        <div className="grid grid-cols-1 gap-2 p-2 sm:grid-cols-2">
          {PainPoints.map((card, i) => (
            <div
              key={i}
              className="bg-background border-border flex flex-col justify-between rounded-2xl border p-4 shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.102),0px_10px_15px_-3px_rgba(0,0,0,0.102)] sm:p-6"
            >
              <div className="flex w-full items-start justify-between p-4 sm:p-6">
                <p className="text-lg font-bold tracking-tight sm:text-2xl md:text-4xl">{card.title}</p>
                {card.icon && (
                  <card.icon className="text-primary h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20" strokeWidth={1.5} />
                )}
              </div>
              <div className="w-full max-w-xl p-4 sm:p-6">
                <p className="w-full max-w-96 text-sm font-bold tracking-tight sm:text-base md:text-xl">{card.desc}</p>
                <p className="pt-2 text-xs tracking-tighter sm:text-sm md:text-base">{card.sub}</p>
              </div>
            </div>
          ))}
        </div>
