    ...
    <div className="relative w-full flex-col">
      <Tabs defaultValue={tabsData?.[0]?.value ?? 'industry'} className="flex w-full flex-col gap-8 px-4 sm:px-0">
        <FullBleedLines className="overflow-hidden">
          <TabsList className="bg-foreground/5 flex min-h-16 gap-5 overflow-x-auto rounded-none px-3 py-2 whitespace-nowrap sm:overflow-visible">
            {tabsData.map(tab => {
              const Icon = tab.icon;
              return (
                <TabsTrigger
                      ...
                      <div className="px-2 py-4">
                        <div className="flex flex-1 flex-col items-start gap-4 sm:gap-6">
                          <div className="flex w-full flex-col gap-2">
                            <b className="font-heading text-xl leading-tight tracking-tight sm:text-2xl">
                              {item.title}
                            </b>
